...
YAAMP_JOB_TEMPLATE *coind_create_template(YAAMP_COIND *coind)
{
	...
	if(!strcmp(g_stratum_algo, "x16rt")) {
		...
		const char *denom100 = json_get_string(json_accumulatorhashes, "100");
		strcpy(templ->denom100, denom100 ? denom100 : "");

		const char *denom1000 = json_get_string(json_accumulatorhashes, "1000");
		strcpy(templ->denom1000, denom1000 ? denom1000 : "");

		const char *denom10000 = json_get_string(json_accumulatorhashes, "10000");
